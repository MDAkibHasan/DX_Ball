...
public class Level2 implements Initializable {
    ...
    public void nextLevel(ActionEvent event ) throws IOException {
        ...
        root=loader.load();
        musics.stop();
        Level3 level3=loader.getController();
        level3.setSound(gameSoundOn,musicSoundOn);
        level3.inheritInfo(score,lives);
    ...
    AudioClip musics = new AudioClip("file:src/main/resources/parent/Level2_music.wav");
    ...
    public void createBricks(){
        ...
        for (double i = height; i >50; i = i - 50) {
            ...
            k++;
            for (double j = width-x; j >x ; j = j - 25) {
                if(spaceCheck % 2== 0){
                    ...
                    if(num==1)
                        rectangle.setFill(Color.MINTCREAM);
                    else  if( num==2)
                        rectangle.setFill(Color.SILVER);
                    else if(num==3)
                        rectangle.setFill(Color.POWDERBLUE);
                    else if(num==4)
